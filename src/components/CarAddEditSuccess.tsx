...
type CarEditSuccessProp = {
    context: {
        isShowingSavingSuccess: boolean
        htmlFor: 'add' | 'edit'
    }
}

const CarEditSuccess: FC<CarEditSuccessProp> = ({context}) => {
    const {isShowingSavingSuccess, htmlFor} = context
    
    const navigate = useNavigate()
    return (
        <div className={`${isShowingSavingSuccess ? 'flex' : 'hidden'} h-screen w-screen top-0 left-0 fixed justify-center items-center`} style={{backgroundColor: 'rgba(0, 0, 0, 0.5)'}}>
            <div className="w-88 h-88 p-6 flex-col flex items-center justify-center bg-white rounded-md shadow-md gap-2 sm:w-68">
                ...
                <p className="font-bold mt-2">Yeay!</p>
                <p>Berhasil {htmlFor === 'add' ? 'menambahkan' : 'mengedit'} data mobil.</p>
                <div className="flex font-bold mt-2">
                    <button onClick={() => navigate('/admin/dashboard') } className="bg-bcr-success text-white rounded-md px-3 py-2">Kembali ke Dashboard</button>
                </div>
            </div>
        </div>
    )
